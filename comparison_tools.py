...
def create_regional_comparison(
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            categoryorder="array",
            categoryarray=pivot.index.tolist()[::-1] if get_chart_sort_order() == "desc" else pivot.index.tolist(),
            fixedrange=True,
        ),
        barmode='stack',
    ...
    if prev_period and not df[df[period_col] == prev_period].empty:
        ...
        fig2.update_layout(
            ...
            yaxis=dict(
                title="", tickfont=dict(size=11),
                categoryorder="array",
                categoryarray=pivot_growth.index.tolist()[::-1] if get_chart_sort_order() == "desc" else pivot_growth.index.tolist(),
                fixedrange=True,
            ),
