...
with tab_home:
    ...
    st.markdown("Бърз поглед към ключовите показатели и трендове.")
    ...
    with col1:
        ...
        st.metric(
            ...
            help="Общо продадени опаковки във филтрираните данни"
        )
    
    with col2:
        total_products = df_filtered["Drug_Name"].nunique()
        st.metric(
            "Продукти",
            f"{total_products}",
            help="Брой уникални продукти в данните"
        )
    ...
    with col3:
        ...
        st.metric(
            "Региони",
            f"{total_regions}",
            help="Брой региони с данни"
        )
    
    st.divider()
    
    # Top performers (по Units)
    st.subheader("🏆 Топ 5 продукта (по продажби)")
    top_products = df_filtered.groupby("Drug_Name")["Units"].sum().sort_values(ascending=False).head(5)
    ...
    if len(periods) >= 2:
        ...
        # Units по продукт за последните 2 периода
        df_growth = df_filtered[df_filtered["Quarter"].isin([last_period, prev_period])]
    ...
    st.subheader("⚠️ Продукти с отрицателен ръст")
    
    if len(periods) >= 2:
        df_decline = df_filtered[df_filtered["Quarter"].isin([last_period, prev_period])]
        ...
        if prev_period in decline_pivot.columns and last_period in decline_pivot.columns:
            ...
            if not decline_pivot.empty:
                ...
                st.success("Няма продукти с отрицателен ръст!")
    else:
        st.info("Нужни са поне 2 периода за изчисляване на ръст.")
